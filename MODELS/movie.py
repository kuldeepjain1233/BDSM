from collaborativefiltering import CollaborativeRecommender, preprocessBooks, preprocessMovies
from tkinter import *
...
def movie():
    ...
    def show_dropdown():
        ...
        options = recommender.find_closest_matches(moviesearch)
        # print(options)
        ...
        dropdown = ttk.OptionMenu(
            dropdown_window, selected_option, *options, command=on_movie_selected)
        dropdown.pack()

    selected_rating = tk.StringVar(base)
    selected_rating.set("1")
    rating_dropdown = ttk.OptionMenu(
        base, selected_rating, "1", "2", "3", "4", "5", command=on_rating_clicked)
    ...
    ttk.Button(base, text="Click to enter preferences",
               command=myClick).place(relx=.7, rely=.5, anchor=CENTER)
